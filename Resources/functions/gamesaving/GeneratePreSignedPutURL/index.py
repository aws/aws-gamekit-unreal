...
def lambda_handler(event, context):
    ...
    if not is_valid_primary_identifier(slot_name):
        print(f'Malformed slot_name: {slot_name} provided for player_id: {player_id}')
    ...
    if not is_valid_base_64(metadata):
        print(f'Malformed metadata provided, expected a Base64 encoded string. Metadata: {metadata}')
    ...
    if len(sha_hash) != BASE_64_ENCODED_SHA_256_BYTES or not sha_hash.isascii():
        print(f'Malformed SHA-256 hash: {sha_hash} provided. Must be 44 characters and Base64 encoded.')
