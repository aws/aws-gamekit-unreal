...
class TestIndex(TestCase):
    ...
    def test_can_generate_presigned_url_with_overridden_default_parameters(self, mock_get_table: MagicMock):
        # Arrange
        event = self.get_lambda_event()
        event['headers']['metadata'] = '{"description": "foo_description"}'
    ...
    def test_lambda_returns_a_400_error_code_when_the_metadata_parameter_exceeds_the_limit(self, mock_get_table: MagicMock):
        sub_tests = [
            ('one byte exceeding the limit', 'a', 1, index.MAX_METADATA_BYTES + 1, 400),
            ('exactly at the limit', 'a', 1, index.MAX_METADATA_BYTES + 0, 200),
            ('one byte less than the limit', 'a', 1, index.MAX_METADATA_BYTES - 1, 200),
        ]

        for test_name, character, byte_length_of_one_character, number_of_bytes, expected_http_status_code in sub_tests:
            with self.subTest(test_name):
                # Arrange
                event = self.get_lambda_event()
                context = None
                ...
                metadata = character * number_of_bytes
                event['headers']['metadata'] = metadata
