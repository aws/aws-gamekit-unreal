...
private:
    ...
    void CallGetUserApi();
    void OnGetUserInfoRecieved(const IntResult& result, const FString& userInfo);
    ...
    void OnCompleteIdentityGetAccessToken(const IntResult& result, const FString& accessToken);

    UPROPERTY(Transient, VisibleInstanceOnly, Category = "8. Open Facebook Login", DisplayName = "Return Value:")
    FString FacebookSessionAccessToken;
