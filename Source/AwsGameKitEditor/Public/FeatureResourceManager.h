...
private:
    ...
    IntResult ProcessFeatureVariables(GAMEKIT_FEATURERESOURCES_INSTANCE_HANDLE resourceInstance, FeatureType featureType, TFunction<unsigned int(GAMEKIT_FEATURERESOURCES_INSTANCE_HANDLE resourceInstance, const char** varKeys, const char** varValues, int numKeys)> variablesHandler);
...
public:
    ...
    IntResult GenerateFeatureTemplates(FeatureType featureType);
    IntResult UpdateFeatureParameters(FeatureType featureType);
    ...
    FString GetLastUsedEnvironment(const FString& gameName) const;
