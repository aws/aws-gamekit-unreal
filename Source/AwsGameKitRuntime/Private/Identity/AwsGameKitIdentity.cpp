...
void AwsGameKitIdentity::PollAndRetrieveFederatedTokens(const FPollAndRetrieveFederatedTokensRequest& Request, TAwsGameKitDelegateParam<const IntResult&, const FederatedIdentityProvider_E&> ResultDelegate)
{
    InternalAwsGameKitRunLambdaOnWorkThread([=]
    {
        ...
        FGraphEventRef OrderedWorkChain;
        identityLibrary.IdentityWrapper->GameKitPollAndRetrieveFederatedTokens(identityLibrary.IdentityInstanceHandle, AwsGameKitIdentityTypeConverter::ConvertProviderEnum(Request.IdentityProvider), TCHAR_TO_UTF8(*Request.RequestId), Request.Timeout);
        InternalAwsGameKitRunDelegateOnGameThread(OrderedWorkChain, ResultDelegate, IntResult{}, Request.IdentityProvider);
    });
}

void AwsGameKitIdentity::GetFederatedAccessToken(const FederatedIdentityProvider_E& IdentityProvider, TAwsGameKitDelegateParam<const IntResult&, const FString&> ResultDelegate)
{
    InternalAwsGameKitRunLambdaOnWorkThread([=]
    {
        IdentityLibrary identityLibrary = GetIdentityLibraryFromModule();
        ...
        IntResult result(identityLibrary.IdentityWrapper->GameKitGetFederatedAccessToken(identityLibrary.IdentityInstanceHandle, AwsGameKitIdentityTypeConverter::ConvertProviderEnum(IdentityProvider), &getUserInfoDispatcher, GetUserInfoDispatcher::Dispatch));
...
void AwsGameKitIdentity::GetUser(TAwsGameKitDelegateParam<const IntResult&, const FString&> ResultDelegate)
{
    InternalAwsGameKitRunLambdaOnWorkThread([=]
    {
        FGraphEventRef OrderedWorkChain;
        ...
        auto getUserInfoDispatcher = [&](const char* response)
        {
            userInfo = UTF8_TO_TCHAR(response);
        };
        typedef LambdaDispatcher<decltype(getUserInfoDispatcher), void, const char*> GetUserInfoDispatcher;
        ...
        InternalAwsGameKitRunDelegateOnGameThread(OrderedWorkChain, ResultDelegate, result, userInfo);
    });
}
