...
DefaultFileActions::DefaultFileActions()
{
    ...
    fileWriteDispatchReceiver = writeDesktopFile;
    fileReadDispatchReceiver = readDesktopFile;
    fileSizeDispatchReceiver = getDesktopFileSize;
}
...
{
    auto dispatcher = static_cast<bool(*)(const FString & filePath, const TArray<uint8>&data)>(dispatchReceiver);
...
{
    auto dispatcher = static_cast<bool(*)(const FString & filePath, TArray<uint8>&data)>(dispatchReceiver);

    TArray<uint8> buffer;
    FString filePathFString(UTF8_TO_TCHAR(filePath));
    bool result = dispatcher(filePathFString, buffer);
...
{
    auto dispatcher = static_cast<int64(*)(const FString & filePath)>(dispatchReceiver);
    FString filePathFString(UTF8_TO_TCHAR(filePath));
    return dispatcher(filePathFString);
}
