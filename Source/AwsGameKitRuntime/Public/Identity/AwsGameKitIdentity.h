...
public:
     ...
     * You can call GetFederatedAccessToken() to check if the login was successful.
     *
     * @param Request A struct containing all parameters required to call this method.
     ...
     * The delegate's ::IntResult parameter is a GameKit status code and indicates the result of the API call. Status codes are defined in AwsGameKitErrors.h.
    ...
    /**
     * @brief Get the player's authorized access token for the specified federated identity provider.
     *
     * @details The returned access token will be empty if the player is not logged in with the federated identity provider.
     *
     * @param IdentityProvider The federated identity provider to get the player's access tokens for.
     * @param ResultDelegate The delegate to invoke when this method has completed. The delegate's **FString parameter** is the player's authorized access token,
     ...
     * - GAMEKIT_ERROR_INVALID_FEDERATED_IDENTITY_PROVIDER: The specified federated identity provider is invalid or is not yet supported.
     */
    static void GetFederatedAccessToken(const FederatedIdentityProvider_E& IdentityProvider, TAwsGameKitDelegateParam<const IntResult&, const FString&> ResultDelegate);
     ...
     * the result of the API call. Status codes are defined in AwsGameKitErrors.h. This method's possible status codes are listed below:
     * - GAMEKIT_SUCCESS: The API call was successful.
     * - GAMEKIT_ERROR_LOGIN_FAILED: The player is not logged in.
     * - GAMEKIT_ERROR_GET_USER_FAILED: Either (1) The backend HTTP request failed. Check the output logs to see what the HTTP response code was. Or (2) The backend returned
     * a malformed JSON payload. This should not happen. If it does, it indicates there is a bug in the backend code.
     */
    static void GetUser(TAwsGameKitDelegateParam<const IntResult&, const FString&> ResultDelegate);
};
