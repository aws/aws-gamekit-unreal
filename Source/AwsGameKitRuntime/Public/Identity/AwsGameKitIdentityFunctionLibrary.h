...
public:
    ...
    /**
     * Get the player's authorized access token for the specified federated identity provider.
     *
     * The returned access token will be empty if the player is not logged in with the federated identity provider.
     *
     * @param IdentityProvider The federated identity provider to get the player's access tokens for.
     * @param Results The player's authorized access token, or an empty string if the call failed or the player is not logged in with the federated identity provider.
     * @param Error A GameKit status code indicating the reason the API call failed. Status codes are defined in AwsGameKitErrors.h. This method's possible status codes are listed below:
     * - GAMEKIT_SUCCESS: The API call was successful.
     * - GAMEKIT_ERROR_INVALID_FEDERATED_IDENTITY_PROVIDER: The specified federated identity provider is invalid or is not yet supported.
     */
    UFUNCTION(BlueprintCallable, Category = "AWS GameKit | Identity", meta = (WorldContext = "WorldContextObject", Latent, LatentInfo = "LatentInfo", ExpandEnumAsExecs = "SuccessOrFailure"))
    static void GetFederatedAccessToken(
        UObject* WorldContextObject,
        FLatentActionInfo LatentInfo,
