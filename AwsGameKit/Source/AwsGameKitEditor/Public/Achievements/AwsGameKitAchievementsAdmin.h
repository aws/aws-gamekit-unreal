...
private:
    ...
    static bool CredentialsValid(bool loggingEnabled = true);
    static void GetCredentialsAndInfo(AccountCredentialsCopy& ac, AccountInfoCopy& ai);

    static TSharedPtr<EditorState> editorState;
    static TSharedPtr<EditorState> GetEditorState();
public:
    ...
    */
     ...
     *
     * @details No result delegate as this isn't a blocking method, only possible failure is an invalid region code which will be logged.
    */
    static void ChangeCredentials();

    /**
     * @brief Checks whether the passed in ID has invalid characters or length.
     *
     * @details No result delegate as this isn't a blocking method.
     *
     * @param achievementId The ID you wish to check.
     * @return Returns true if the ID is valid, else false.
    */
    static bool IsAchievementIdValid(const FText& achievementId);
    ...
    */
    static AdminAchievement GetAdminAchievementFromJsonResponse(const TSharedPtr<FJsonObject>& achievementData)
    {
        ...
        AwsGamekitAchievementsResponseProcessor::SetStringField(achievementData, achievement.achievementId, "achievement_id");
        AwsGamekitAchievementsResponseProcessor::SetStringField(achievementData, achievement.title, "title");
        AwsGamekitAchievementsResponseProcessor::SetStringField(achievementData, achievement.lockedDescription, "locked_description");
        AwsGamekitAchievementsResponseProcessor::SetStringField(achievementData, achievement.unlockedDescription, "unlocked_description");
        AwsGamekitAchievementsResponseProcessor::SetStringField(achievementData, achievement.lockedIcon, "locked_icon_url");
        AwsGamekitAchievementsResponseProcessor::SetStringField(achievementData, achievement.unlockedIcon, "unlocked_icon_url");

        AwsGamekitAchievementsResponseProcessor::SetNumberField(achievementData, achievement.requiredAmount, "max_value");
        AwsGamekitAchievementsResponseProcessor::SetNumberField(achievementData, achievement.points, "points");
        AwsGamekitAchievementsResponseProcessor::SetNumberField(achievementData, achievement.sortOrder, "order_number");

        achievement.isStateful = achievement.requiredAmount > 0;
        AwsGamekitAchievementsResponseProcessor::SetBoolField(achievementData, achievement.isSecret, "is_secret");
        AwsGamekitAchievementsResponseProcessor::SetBoolField(achievementData, achievement.isHidden, "is_hidden");
        AwsGamekitAchievementsResponseProcessor::SetBoolField(achievementData, achievement.localLockedIcon, "local_locked_icon");
        AwsGamekitAchievementsResponseProcessor::SetBoolField(achievementData, achievement.localUnlockedIcon, "local_unlocked_icon");
