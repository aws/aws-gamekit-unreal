...
void AAwsGameKitIdentityExamples::OnGetUserInfoRecieved(const IntResult& result, const FGetUserResponse& userInfo)
{
...
void AAwsGameKitIdentityExamples::OnGetFacebookLoginUrlComplete(const IntResult& result, const FLoginUrlResponse& loginInfo)
{
    ...
    FacebookLoginReturnValue = "PLEASE WAIT...";
...
void AAwsGameKitIdentityExamples::OnCompletePollAndRetrieveFederatedTokens(const IntResult& result, const FederatedIdentityProvider_E& identityProvider)
{
    ...
    {
        const auto Delegate = MakeAwsGameKitDelegate(this, &AAwsGameKitIdentityExamples::OnCompleteIdentityGetIdToken);
        AwsGameKitIdentity::GetFederatedIdToken(identityProvider, Delegate);
    }
    else
    {
        UE_LOG(LogAwsGameKit, Display, TEXT("Login Failed with error : %u"), result.Result);
        FacebookLoginReturnValue = GetResultMessage(result.Result);
    }
}

void AAwsGameKitIdentityExamples::OnCompleteIdentityGetIdToken(const IntResult& result, const FString& accessToken)
{
    UE_LOG(LogAwsGameKit, Display, TEXT("AAwsGameKitIdentityExamples::OnCompleteIdentityGetIdToken()"));

    if(result.Result == GameKit::GAMEKIT_SUCCESS)
    {
        FacebookLoginReturnValue = accessToken;
        UE_LOG(LogAwsGameKit, Display, TEXT("Federated (Facebook) Successful: %u"), result.Result);
        UE_LOG(LogAwsGameKit, Display, TEXT("Federated (Facebook) AccessToken: %s"), *accessToken);
    }
    else
    {
        UE_LOG(LogAwsGameKit, Display, TEXT("Login Failed with error : %u"), result.Result);
