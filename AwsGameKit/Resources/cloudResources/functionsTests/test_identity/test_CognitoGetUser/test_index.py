...
# The AWS credentials file and default region are patched in case the user has an invalid credentials file. By patching
# the contents of the credentials file no longer matter.
with patch("boto3.client") as boto_client_mock:
    with patch.dict(os.environ, {"IDENTITY_TABLE_NAME": "gamekit_identity",
                                 "AWS_SHARED_CREDENTIALS_FILE": './fakePath',
                                 "AWS_DEFAULT_REGION": 'us-west-2'}, clear=False):
...
class TestIndex(TestCase):
    ...
    def test_details_can_be_fetched_for_a_user_successfully(self, mock_boto3):
        ...
        index.ddb_table = mock_ddb_table
