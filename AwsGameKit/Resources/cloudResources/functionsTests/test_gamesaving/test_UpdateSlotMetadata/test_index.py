...
class TestIndex(TestCase):
    ...
    def test_dynamo_item_is_not_updated_when_s3_bucket_does_not_exist_raises_exception(self, mock_write_metadata_to_dynamodb: MagicMock):
        # Arrange
        event = self.get_lambda_event()
        context = None
        self.set_mock_s3_object_metadata(EMPTY_METADATA, TEST_LAST_MODIFIED_TIME, index.s3_resource)

        index.s3_resource.Object.return_value = None
        index.s3_client.load.side_effect = new_boto_exception(botocore.exceptions.ClientError)
        index.s3_client.head_bucket.side_effect = new_boto_exception(botocore.exceptions.ClientError)
        ...
        # Assert
        mock_write_metadata_to_dynamodb.assert_not_called()

    @patch('functions.gamesaving.UpdateSlotMetadata.index.write_metadata_to_dynamodb')
    def test_dynamo_item_is_not_updated_when_unknown_error_occurs_raises_exception(self, mock_write_metadata_to_dynamodb: MagicMock):
        # Arrange
        event = self.get_lambda_event()
        context = None
        self.set_mock_s3_object_metadata(EMPTY_METADATA, TEST_LAST_MODIFIED_TIME, index.s3_resource)
        ...
        index.s3_resource.Object.return_value = None
        index.s3_client.load.side_effect = new_boto_exception(botocore.exceptions.ClientError)
        index.gamesaves_table.get_item.return_value = {'Item': {'metadata': 'SSR'}}
    ...
    @patch('functions.gamesaving.UpdateSlotMetadata.index.write_metadata_to_dynamodb')
    def test_dynamo_item_is_not_updated_when_dynamo_record_does_not_exist_exits_gracefully(self, mock_write_metadata_to_dynamodb: MagicMock):
        # Arrange
        event = self.get_lambda_event()
        context = None
        self.set_mock_s3_object_metadata(EMPTY_METADATA, TEST_LAST_MODIFIED_TIME, index.s3_resource)
        ...
        index.s3_resource.Object.return_value = None
        index.s3_client.load.side_effect = new_boto_exception(botocore.exceptions.ClientError)
        index.gamesaves_table.get_item.return_value = {'ResponseMetadata': {'RequestId': '1', 'HTTPStatusCode': 200}}
